# HF Scalper V1 — End User Instructions

> **Version:** 1.0 · **Platform:** TradingView (Pine Script v6) · **Designed for:** 1m – 15m scalping
...
10. [Known limitations & risk disclaimer](#10-known-limitations--risk-disclaimer)

---

## 1. What this indicator does

HF Scalper V1 is a **multi-timeframe confluence engine** for short-duration scalp trades.
Five indicators run silently in the background and vote on direction and strength.
You only see three output layers:

```
┌─────────────────────────────────────────────────────────────┐
│  LAYER 1 – Dashboard table (top-right corner)               │
...
│                                                             │
│  LAYER 3 – TP1 / TP2 / TP3 + SL dotted lines               │
...
3. Paste the full contents of `HF.V1.pine`.
4. Click **Save** (give it a name, e.g. *HF Scalper V1*).
...
The dashboard table appears in the corner you choose (default: top-right).
...
║  M2  ║ ▲ BUY ║  68.1%║
║  M5  ║ ▲ BUY ║  61.5%║
║  M15 ║ ▼ SELL║  41.2%║  ← lower TF disagreement = caution
╠══════╬═══════╬═══════╣
...
dotted/dashed lines are drawn to the right of the last bar:
...
- A Fibonacci extension of the recent price swing (61.8% / 100% / 161.8%)
...
> **Note:** TP/SL lines disappear when confidence drops below the threshold. This is
> intentional — low-confidence signals should not have visible TP targets.
...
| Pivot Lookback | **20** | How many bars to use for swing high/low Fib anchors |
...
> and shows TP lines only on high-conviction setups — reduces noise in choppy markets.
...
you the signal is too weak to project targets reliably.
...
## 10. Known limitations & risk disclaimer
